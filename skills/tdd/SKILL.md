---
name: tdd
description: Vertical-slice test-driven development. ONE test → ONE implementation → repeat. Hard rule against horizontal slicing (writing all tests first, then all impl). Tests verify behavior through public interfaces, not implementation details. USE WHEN tdd, red green refactor, test driven, test-first, write tests, tracer bullet, vertical slice tdd.
---

# Test-Driven Development

Vertical-slice TDD: each cycle is one test → one piece of implementation → repeat. The discipline is **anti-horizontal-slicing** — never write all tests first and then all implementation.

## Philosophy

**Core principle**: Tests verify behavior through public interfaces, not implementation details. Code can change entirely; tests shouldn't.

**Good tests** are integration-style. They exercise real code paths through public APIs. They describe *what* the system does, not *how* it does it. A good test reads like a specification — `"catalog ingests a document without an id"` tells you exactly what capability exists. These tests survive refactors because they don't care about internal structure.
...
- [ ] Confirm which behaviors to test (you can't test everything — pick the critical paths)
...
**Critical**: Confirm exactly which behaviors are in scope. A good rule of thumb — don't solve things that might never be a problem. Focus testing effort on critical paths and complex logic, not every conceivable edge case.
...
```
RED:   Write test for first behavior → test fails (ideally with a clear error)
...
- **One test at a time**
- **Only enough code to pass the current test** — no speculative implementation
- **Don't anticipate future tests** — let them drive the next cycle
...
- Avoid `unittest.mock.patch` on your own internal modules — that couples tests to implementation. See [references/mocking.md](references/mocking.md).
