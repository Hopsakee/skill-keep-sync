...
PARAM_CONFIG: dict[str, dict] = {
    'Q': {
        'description': 'Discharge / debiet (mÃÂ³/s)',
...
def _search_sites_by_name(location: str) -> tuple[pd.DataFrame, str]:
    ...
    tuple[pd.DataFrame, str]
        (df_sites, matched_term) Ã¢ÂÂ empty DataFrame if nothing found.
    """
...
def find_timeseries(
    ...
    if ts_shortname_override:
        ...
        if df_ts_all.empty:
            print(
                f"Warning: no timeseries returned for station '{station_no}' Ã¢ÂÂ "
