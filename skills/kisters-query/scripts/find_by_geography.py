...
def lookup_location(name: str) -> dict:
    ...
    - Name without spaces (e.g. 'Meppeler Diep' Ã¢ÂÂ 'Meppelerdiep')
...
def find_sites_in_area(
    ...
    print(f"CentroÃÂ¯de: {loc['lat']:.4f}ÃÂ°N, {loc['lon']:.4f}ÃÂ°E  (OSM type: {loc['osm_class']})")
    ...
    if width_km > 0.5 or height_km > 0.5:
        ...
        print(
            f"OSM-feature heeft een oppervlak van {width_km:.1f} km ÃÂ {height_km:.1f} km Ã¢ÂÂ "
        ...
        print(
            f"OSM-resultaat is een punt Ã¢ÂÂ alle meetpunten binnen {effective_km:.1f} km "
    ...
    # NOTE: do NOT add parametertype_name here Ã¢ÂÂ the server-side filter misses compound
    ...
    # For area features: KiWIS bbox already scoped to OSM bbox + buffer Ã¢ÂÂ keep all.
...
def _filter_sites_by_parameter(df_sites: pd.DataFrame, parameter: str) -> pd.DataFrame:
    """Phase 1 Ã¢ÂÂ fast: keep only sites that have the given parameter type.
...
def filter_sites_by_coverage(
    ...
    """Phase 2 Ã¢ÂÂ slower: verify actual measurements exist and check date coverage.
...
def main() -> None:
    ...
    if args.parameter or args.from_dt or args.to_dt:
        ...
        # Auto-confirm when the user explicitly specified a small radius (Ã¢ÂÂ¤ 1 km) Ã¢ÂÂ
        ...
        print("(Kan even duren Ã¢ÂÂ per site worden tijdreeksen opgehaald.)\n")
