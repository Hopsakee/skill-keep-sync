...
| **EvalSkill** | "eval skill", "grade skill output", "test until passing", "binary checks" | `Workflows/EvalSkill.md` |
...
### Evals & Grading — the self-improving loop (`Workflows/EvalSkill.md`)

Two axes, run both: **TestSkill** asks "does this skill add value over no skill?"; **EvalSkill** asks "does output still meet a fixed bar after I changed it?" (regression). EvalSkill gives a skill a durable `Evals.md` — ~8–12 **binary** checks grouped by category. Binary, never 1–5: a grader can't reliably resolve 3/5 from 4/5, so a score drifts and can't catch regression; a yes/no check forces you to name the observable behavior.

The one rule that earns its keep: **grade in a fresh context.** The agent that grades must NOT be the one that wrote or improved the skill — an author grades their own homework with privileged knowledge of intent and "sees" quality that isn't on the page. A clean-window grader sees only what a real user sees. Keep the iterate-until-pass loop thin — the durable assets are `Evals.md` + the fresh grade; the retry cascade is a crutch a smarter model erodes (BPE).

**No new memory file.** Subjective taste feedback that resists binarization lands in `## Gotchas` as a `Lessons:` note (not a parallel `Memory.md` — that duplicates Gotchas). AI-slop tells (em-dash overuse, "X, not Y", hollow closers) belong to your writing-style surface (`PAI/USER/AI_WRITING_PATTERNS.md`), optionally as one binary eval check — not a standalone cleanup pass.
