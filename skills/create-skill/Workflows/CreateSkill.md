...
## Step 2c: Examples-First Foundation (output-shaped skills)

If the skill produces a **recurring output artifact** (a report, an email, a blog post, a vault note, a code file in a house style), do NOT write the skill from your head. Collect **≥3 real, best-in-class examples** of that output from the user first, then derive the skill's conventions from them.

Why: examples are *data*, not instructions. The real artifact encodes latent conventions (structure, voice, what's omitted) that no amount of describing-from-imagination recovers — and a smarter model extracts *more* from the same examples, so this only grows more valuable. This is the input-side analog of "the vault wins over training data."

- Ask: "Paste or point me at 3+ examples of the output you want this skill to produce."
- Read all of them; extract the shared shape (sections, order, tone, length, anti-patterns).
- Store the examples as context files in the skill root (`ExampleOne.md`, etc.) or summarize the extracted conventions into the SKILL.md body — keep SKILL.md lean.
- For a `_ALLCAPS` (private) skill the examples may be the user's real artifacts; for a `TitleCase` (public) skill, generalize them or move the real ones to `SKILLCUSTOMIZATIONS/`.

Skip this step for skills that don't produce a stable output artifact (tool wrappers, data-fetchers, runbooks) — there's no output shape to ground.
...
```bash
rg -i "<your-name>|<your-org>|<your-domain>|/Users/[a-z]+/|/home/[a-z]+/" ~/.claude/skills/[SkillName]/
```
