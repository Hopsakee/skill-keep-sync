...
3. **Eval results** (if the skill has an `Evals.md`): Read which binary checks failed — these are the most precise improvement signal because each names a specific observable behavior. See `Workflows/EvalSkill.md`.
4. **The skill's `## Gotchas` + any `Lessons:` notes:** accumulated failure knowledge and subjective taste feedback from prior uses live here (NOT in a separate memory file). Read them before changing anything.
5. **User feedback:** What specific complaints or requests did the user have?
6. **Transcripts** (if available): How did the agent actually use the skill? Where did it waste time or go wrong?
...
| "A behavior we'd fixed broke again" | No regression contract | Add a binary check to `Evals.md` (see EvalSkill) |
| "Reads like AI slop" (em-dashes, "X, not Y", hollow closers) | Style drift | Point at `PAI/USER/AI_WRITING_PATTERNS.md`; optionally encode one binary slop check in `Evals.md`. Don't add a standalone cleanup pass — it duplicates the style guide and a smarter model erodes it. |
...
- **If the skill has an `Evals.md` — this is a BLOCKING regression gate, not a suggestion.** Regenerate output and re-grade it in a **fresh context** (a grader subagent that did NOT see this improvement work — see `Workflows/EvalSkill.md`). **All checks must pass before the improvement is declared done.** You just edited the skill, so you're the worst-positioned judge of whether the edit worked; a clean-window grader sees only what a user would. A re-runnable contract that nothing re-runs is an orphaned artifact — the "skill gets better over its lifetime" payoff comes entirely from enforcing this gate on every change.
...
- **Subjective taste feedback** ("make the voice more authentic"). Ordered routing, not either/or: **first** try to promote it to a binary check in `Evals.md` (durable + mechanically re-checked); **only if** it's too fuzzy to be binary, drop it here as a `Lessons:` line. Default toward promote-to-eval — value leaks toward the soft channel otherwise. Never create a parallel `Memory.md`; that duplicates this section.
